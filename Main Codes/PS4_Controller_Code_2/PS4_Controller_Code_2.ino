...
const int Long_auto =17;// turn on/off turbo
...
void blower(float speed_value){
    ...
    ledcWrite(pwmCh_c2, max_speed_vacuum*speed_value);
}
...
void setup() {
  ...
  pinMode(Long_auto,OUTPUT);
  digitalWrite(Long_auto,LOW);
...
void loop() {
   ...
   if (PS4.isConnected()) {
      ...
      if (PS4.Cross()){// when cross button is pressed it does an automated sequence of throw the trash and returning the trash collector to the collection position.
  
        throw_trash(.9);
  
        delay(600);
        ledcWrite(pwmCh_c1, 0);
        delay(2500);
        ...
        digitalWrite(c1_pwmL,HIGH);
        digitalWrite(c1_pwmR,LOW);
        ledcWrite(pwmCh_c1, 220);
      }

      //if the right joystick is moved up, the trash collector is moved to the collection position. The joystick value is divided by 128 to map the values from 0 to 1.
      else if (PS4.RStickY()/128.0 > 0.1){
        collect_trash(abs(PS4.RStickY())/128.0);
        Serial.printf("Right Stick: %f\n",(PS4.RStickY()/128.0) );
      }
      //if the right joystick is moved down, the trash collector is moved to the throwing position. The joystick value is divided by 128 to map the values from 0 to 1.
      else if (PS4.RStickY()/128.0 < -0.1){
        throw_trash(abs(PS4.RStickY())/128.0);
        Serial.printf("Right Stick: %f\n",(PS4.RStickY()/128.0) );
      }
      //if none of the above is done set the trash position motors to 0.
      else{
        ledcWrite(pwmCh_c1, 0);
      }

      //if either of the trigger button is pressed calculate the vacuum value, then calls the blow and suction functions based on the value of the vacuum calculated.
      if (PS4.L2() || PS4.R2()){
  
        float vacuum = ((PS4.R2Value()/256.0) - (PS4.L2Value()/256.0));
        
         if (vacuum <-0.1 &&(PS4.L2() || PS4.R2()) ){
           sucker(abs(vacuum));
           Serial.printf("sucker");
         }
         else if (vacuum >0.1 && (PS4.L2() || PS4.R2())){
           blower(abs(vacuum));
           Serial.printf("Blower");
         }
        Serial.printf("L2 pressed: %f\n",(PS4.L2Value()/256.0));
        Serial.printf("R2 pressed: %f\n",(PS4.R2Value()/256.0));
        Serial.println(vacuum);
        }
         //if non of the trigger button is pressed set the vacuum motor speed to 0.  
        else{
          ledcWrite(pwmCh_c2, 0);
         } 

      if (PS4.LStickY()/128.0 > 0.1){
        tele_extend(abs(PS4.LStickY())/128.0);
        Serial.printf("Left Stick: %f\n",(PS4.LStickY()/128.0) );
      }
      //if the right joystick is moved down, the trash collector is moved to the throwing position. The joystick value is divided by 128 to map the values from 0 to 1.
      else if (PS4.LStickY()/128.0 < -0.1){
        tele_retract(abs(PS4.LStickY())/128.0);
        Serial.printf("Left Stick: %f\n",(PS4.LStickY()/128.0) );
      }
      //if none of the above is done set the trash position motors to 0.
      else{
