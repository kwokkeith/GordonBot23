...
const int PWM_FL_CH = 0;
const int PWM_FR_CH = 1;
const int PWM_BL_CH = 2;
const int PWM_BR_CH = 3;
...
void setup()
{
  ...
  ledcAttachPin(FL_EN, PWM_FL_CH);
  ledcSetup(PWM_FL_CH, freq, resolution);

  ledcAttachPin(FR_EN, PWM_FR_CH);
  ledcSetup(PWM_FR_CH, freq, resolution);

  ledcAttachPin(BL_EN, PWM_BL_CH);
  ledcSetup(PWM_BL_CH, freq, resolution);

  ledcAttachPin(BR_EN, PWM_BR_CH);
  ledcSetup(PWM_BR_CH, freq, resolution);
...
void loop()
{
  ...
  if (millis() % 10 == 0 ) {
    ...
    float rotationpower_CCW;
    rotationpower_CCW = -(channel_values[7] - 1500.0) / 500.0;
    ...
    if (power > deadzone || abs(rotationpower_CCW) > deadzone) {
      ...
      ledcWrite(PWM_FL_CH, 0);
      ledcWrite(PWM_FR_CH, 0);
      ledcWrite(PWM_BL_CH, 0);
      ledcWrite(PWM_BR_CH, 0);
    }
...
void move_robot(float power, float Dir_power_FL, float Dir_power_FR, float Dir_power_BL, float Dir_power_BR, float power_CCW) {
  ...
  if (power > deadzone) {
    ...
    if (Dir_power_FL > 0) {
      ...
      ledcWrite(PWM_FL_CH, adj_power * trim_left);
    }
    ...
    if (Dir_power_FR > 0) {
      ...
      ledcWrite(PWM_FR_CH, adj_power * trim_right);
    }
    ...
    if (Dir_power_BL > 0) {
      ...
      ledcWrite(PWM_BL_CH, adj_power * trim_left);
    }
    ...
    if (Dir_power_BR > 0) {
      ...
      ledcWrite(PWM_BR_CH, adj_power * trim_right);
    }
    ...
    if (power_CCW > 0) {
      ...
      digitalWrite(FR_PWM, HIGH);
      ledcWrite(PWM_FR_CH, min_to_start + power_CCW * (255 - min_to_start) * trim_right);
      digitalWrite(BR_PWM, LOW);
      ledcWrite(PWM_BR_CH, min_to_start + power_CCW * (255 - min_to_start) * trim_right);

      digitalWrite(FL_PWM, LOW);
      ledcWrite(PWM_FL_CH, min_to_start + power_CCW * (255 - min_to_start) * trim_left);
      digitalWrite(BL_PWM, HIGH);
      ledcWrite(PWM_BL_CH, min_to_start + power_CCW * (255 - min_to_start) * trim_left);
    }
    else {
      //Rotation Clockwise
      digitalWrite(FL_PWM, HIGH);
      ledcWrite(PWM_FL_CH, power_CCW * 255 * trim_left);
      digitalWrite(BL_PWM, LOW);
      ledcWrite(PWM_BL_CH, power_CCW * 255 * trim_left);

      digitalWrite(FR_PWM, LOW);
      ledcWrite(PWM_FR_CH, power_CCW * 255 * trim_right);
      digitalWrite(BR_PWM, HIGH);
      ledcWrite(PWM_BR_CH, power_CCW * 255 * trim_right);
    }
  }
